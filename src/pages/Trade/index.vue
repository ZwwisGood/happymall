  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="detail">
        ...
        >
          <li style="width: 10%">
            <img :src="order.imgUrl" alt="" style="width: 110px" />
          </li>
          <li style="width: 40%; margin-right: 50px">
            <p>
              {{ order.skuName }}
            </p>
            <h4>7天无理由退货</h4>
          ...
          <li style="width: 5%">× {{ order.skuNum }}</li>
        </ul>
      </div>
      <div class="bbs">
        <h5>买家留言：</h5>
...
export default {
  ...
  methods: {
    ...
    changeDefault(address, addressInfo) {
      console.log(this.addressInfo.find((item) => (item.isDefault = 1)))
    ...
    //价格取两位小数
    price(order) {
      if (!order.orderPrice) return ''
      return order.orderPrice.toFixed(2)
    },
    //提交订单
    async submitOrder() {
      //需要带的参数：tradeNo交易编码
  ...
  computed: {
    ...
    //计算商品总价，保留两位小数
    totalAmount() {
      if (!this.orderInfo.totalAmount) return ''
      return this.orderInfo.totalAmount.toFixed(2)
    },
  },
}
</script>
