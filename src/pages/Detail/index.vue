        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              ...
              <div class="add">
                <a @click="addShopcar" href="javascript:">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  computed: {
    ...
    skuImageList() {
      //Zoom组件挂载完毕后可能服务器数据还没有回来，所以skuInfo为undefined
      //所以默认给一个空数组（skuImageList是个数组），这样就不会报假错误
  ...
  methods: {
    ...
    async addShopcar() {
      ...
      //2.你需要知道这次请求成功还是失败，如果成功则进行路由跳转；如果失败，给用户提示
      try {
        //成功
        ...
        //3.路由跳转
        //4.在路由跳转的时候将产品的信息带给下一级路由组件
        //下面这种方法可以，但是地址栏不显示中文
        // this.$router.push({name:'addcartsuccess',query:{skuInfo:this.skuInfo,skuNum:this.skuNum}})
        //一些简单的数据，如skuNum，通过query形式给路由组件传递过去
        //复杂的数据，如skuInfo产品信息（对象），通过会话存储传递（会话结束就消失）
        //本地存储、会话存储一般存储的是字符串，需要使用JSON.stringfy()转换为JSON格式数据
